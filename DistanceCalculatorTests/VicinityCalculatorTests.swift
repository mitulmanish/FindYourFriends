//
//  VicoinityCalculatorTests.swift
...
class VicinityCalculatorTests: XCTestCase {
    private let customersDictionarylist = [
    ...
    private let clLocationDistanceCalculator: DistanceComputation = CLLocationDistanceCalculator()
    private let haversineDistanceCalculator: DistanceComputation = HaversineDistanceCalculator()
    private let headquartersLocation: LocationCoordinate = LocationCoordinate(latitude: 53.339428, longitude: -6.257664)

    func test_haversine_distance_calculator_produces_same_guest_list_as_CLLocation_distance_calculator() {
        let distanceCalculator1 = VicinityCalculator(sourceCoordinate: headquartersLocation, customers: givenCustomerList(), distanceCalculator: haversineDistanceCalculator)
        
        let distanceCalculator2 = VicinityCalculator(sourceCoordinate: headquartersLocation, customers: givenCustomerList(), distanceCalculator: clLocationDistanceCalculator)
        
        let guestList1 = distanceCalculator1.computeGuestList(within: 100.0)
        let guestList2 = distanceCalculator2.computeGuestList(within: 100.0)
        
        for (index, guest) in guestList1.enumerated() {
            XCTAssertEqual(guest.userID, guestList2[index].userID)
            XCTAssertEqual(guest.userName, guestList2[index].userName)
        }
    }

    func givenCustomerList() -> [Customer] {
        return customersDictionarylist.compactMap { try? Customer(from: $0) }
    }
}
